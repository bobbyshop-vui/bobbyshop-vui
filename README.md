...
## 7. Using `main.nim` — the CLI training driver

`main.nim` is a runnable program (`when isMainModule: main()`) that uses the
exact API of the four library files above to implement a complete training
loop: a byte-level tokenizer, data loading, training, checkpoint + resume,
and quantization when saving the final model.
...
|---|---|---|
| `--seq` | 128 | Sequence length per sample |
| `--batch` | 32 | Batch size |
...
| `--ff-mult` | 4 | Feed-forward expansion factor |
| `--requantize-every` | 50 | Every how many Adam steps APF re-quantizes the parameter |
| `--save` | finetune.nimq | Path to the final checkpoint |
| `--tokenizer` | tokenizer.json | Tokenizer file path (save/load) |
| `--data-dir` | . | Directory containing data (e.g. `databricks-dolly-15k.jsonl`) |
| `--quant` | auto | Compression kind for the final checkpoint: `int8`\|`int4`\|`fp8`\|`fp8_e5m2`\|`auto`\|`none` |
| `--ckpt-every` | 5 | Every how many steps a temporary checkpoint (`--save`+`.ckpt`) is saved for resuming |
| `--log-every` | 10 | Every how many steps to print the loss |
| `--stockfish-path` | /usr/local/bin/stockfish | Path to the Stockfish binary (self-play data source, skipped if absent) |
| `--stockfish-games` | 40 | Number of Stockfish self-play games |
| `--stockfish-plies` | 10 | Number of plies per game |
| `--so-tags` | python,c,swift,objective-c,nim | StackOverflow tags to pull questions from |
| `--so-max-pages` | 100 | Max pages to fetch from the StackOverflow API |
| `--wiki-max-pages` | 10 | Max Wikipedia pages to fetch |
| `--seed` | 1337 | Random seed |
| `--help` / `-h` | — | Print usage help |

**Automatic resume:** if the file `<--save>.ckpt` already exists from a
previous run, `main.nim` automatically detects it and reloads the model +
optimizer (m/v) + step instead of training from scratch. The tokenizer is
also reloaded from `--tokenizer` to preserve the original id↔byte mapping.

**Data sources:** currently `main.nim` only enables
`databricks-dolly-15k.jsonl` (read from `--data-dir`). Other loaders
(`grammar.txt`, `million_games.pgn`, `english_words.txt`, Wikipedia,
StackOverflow, Stockfish self-play) already have functions defined but are
disabled in `loadAllTexts()` — to re-enable one, add the corresponding
`result.add load...(...)` line in that function.
